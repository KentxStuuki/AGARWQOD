<div class="relative">
    <!-- Top Background Image -->
    <img src="../main/images/financial_section/tf.png" alt=""
        class="absolute right-0 max-h-[20vh] sm:max-h-[35vh] h-auto w-auto z-0">

    <!-- Content -->
    <div class="relative z-10 py-11">
        <h4
            class="bg-white text-center w-full sm:w-[70%] md:w-[50%] mx-auto py-3 shadow-2xl rounded-full font-semibold mb-10 text-2xl sm:text-3xl text-[var(--primary)]">
            Financial Plan
        </h4>

        <div class="overflow-x-auto w-full sm:w-[90%] md:w-[60%] p-4 sm:p-6 shadow-2xl rounded-2xl mx-auto bg-white">
            <table class="min-w-full border border-black text-[var(--primary)] text-sm sm:text-base md:text-lg">
                <thead>
                    <tr class="text-center">
                        <th class="border border-black px-2 sm:px-4 py-2 sm:py-3">Category</th>
                        <th class="border border-black px-2 sm:px-4 py-2 sm:py-3">Amount (₱)</th>
                    </tr>
                </thead>
                <tbody>
                    <tr>
                        <td class="border border-black px-2 sm:px-4 py-2">Cost of 10 Agarwood Seedlings</td>
                        <td class="border border-black px-2 sm:px-4 py-2 text-right">100,000</td>
                    </tr>
                    <tr>
                        <td class="border border-black px-2 sm:px-4 py-2">Labor and Maintenance (5 years)</td>
                        <td class="border border-black px-2 sm:px-4 py-2 text-right">30,000</td>
                    </tr>
                    <tr>
                        <td class="border border-black px-2 sm:px-4 py-2">Soil Preparation & Farm Inputs</td>
                        <td class="border border-black px-2 sm:px-4 py-2 text-right">10,000</td>
                    </tr>
                    <tr>
                        <td class="border border-black px-2 sm:px-4 py-2">Monitoring/Reporting/Admin Costs</td>
                        <td class="border border-black px-2 sm:px-4 py-2 text-right">10,000</td>
                    </tr>
                    <tr class="font-semibold bg-gray-50">
                        <td class="border border-black px-2 sm:px-4 py-2">Total Project Cost</td>
                        <td class="border border-black px-2 sm:px-4 py-2 text-right">₱ 150,000</td>
                    </tr>
                </tbody>
            </table>
        </div>
    </div>

    <!-- Bottom Background Image -->
    <img src="../main/images/financial_section/bf.png" alt=""
        class="absolute bottom-0 left-0 max-h-[20vh] sm:max-h-[35vh] h-auto w-auto z-0">
</div>